    ...
    for i, sample in enumerate(progress_bar):
        ...
        for i in range(batch_size):
            for j in range(args.beam_size):
                ...
                # Calculate squared log probability
                squared_log_p = log_p ** 2
                ...
                node = BeamSearchNode(searches[i], emb, lstm_out, final_hidden, final_cell,
                                      mask, torch.cat((go_slice[i], next_word)), log_p, 1, squared_log_p)
                # __QUESTION 3: Why do we add the node with a negative score?
                searches[i].add(-node.eval(args.alpha, lambda_regularizer=0.5), node)
        ...
        for _ in range(args.max_len-1):
            ...
            for i in range(log_probs.shape[0]):
                for j in range(args.beam_size):
                    ...
                    # Calculate the squared log probabilities
                    squared_log_p = log_p ** 2
                    ...
                    if next_word[-1] == tgt_dict.eos_idx:
                        node = BeamSearchNode(
                            ...
                            next_word)), node.logp, node.length, node.squared_sum + squared_log_p
                            ) # Update squared_sum
                        search.add_final(-node.eval(args.alpha, lambda_regularizer=0.5), node)
                    ...
                    # Add the node to current nodes for next iteration
                        ...
                        node = BeamSearchNode(
                            search, node.emb, node.lstm_out, node.final_hidden,
                            node.final_cell, node.mask, torch.cat((prev_words[i][0].view([1]),
                            next_word)), node.logp + log_p, node.length + 1, node.squared_sum + squared_log_p
                            ) # Update squared_sum
                        search.add(-node.eval(args.alpha, lambda_regularizer=0.5), node)
