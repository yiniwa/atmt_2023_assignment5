...
def get_args():
    ...
    parser.add_argument('--output', default='model_translations', type=str,
                        help='path to the base output file destination')
    ...
    parser.add_argument('--n-best-list', default='n_best_list', type=str,
                        help='path to the output file for the N-best list')
...
def main(args):
    ...
    for i, sample in enumerate(progress_bar):
        ...
        # Initialize all_hyps with an empty list for each sentence ID in the current batch
        for sent_id in sample['id'].numpy():
            n_best_list[int(sent_id)] = []

        # Create a beam search object or every input sentence in batch
        batch_size = sample['src_tokens'].shape[0]
        searches = [BeamSearch(args.beam_size, args.max_len - 1, tgt_dict.unk_idx, gamma=args.gamma) for i in range(batch_size)]

        # Initialize N-best hypotheses list
        n_best_hyps = [[] for _ in range(batch_size)]
        ...
        for i in range(batch_size):
            for j in range(args.beam_size):
                ...
                searches[i].add(-node.eval(args.alpha), node, rank=j)
                ...
                for j in range(args.beam_size):
                    ...
                    # Add the node to current nodes for next iteration
                        ...
                        node = BeamSearchNode(
                            search, node.emb, node.lstm_out, node.final_hidden,
                            node.final_cell, node.mask, torch.cat((prev_words[i][0].view([1]),
                                                                   next_word)), node.logp + log_p, node.length + 1, rank=j
                        )
                        search.add_final(-node.eval(args.alpha), node)

            # Apply diversity promoting strategy by selecting the best sibling
        ...
        # best_sents = torch.stack([search.get_n_best()[1].sequence[1:].cpu() for search in searches])

        # Iterate over the search objects to get the best nodes
        for j, search in enumerate(searches):
            # Get N-best hypotheses for the current sentence
            n_best_nodes = search.get_n_best(n=args.n_best)
            sentence_id = int(sample['id'][j])

            # Store the N-best hypotheses
            for rank, (score, node) in enumerate(n_best_nodes):
                sentence = node.sequence[1:].cpu().numpy()
                first_eos = np.where(sentence == tgt_dict.eos_idx)[0]
                if len(first_eos) > 0:
                    sentence = sentence[:first_eos[0]]
                sentence_str = tgt_dict.string(sentence)
                # Note: The score here should be the total score (including any penalties)
                n_best_list[sentence_id].append(
                    (node.total_score, sentence_str))  # node.total_score should include diversity penalty

            # Store the best hypothesis for final output
            best_score, best_node = n_best_nodes[0]  # Assuming the first one is the best
            best_sentence = best_node.sequence[1:].cpu().numpy()
            first_eos = np.where(best_sentence == tgt_dict.eos_idx)[0]
            if len(first_eos) > 0:
                best_sentence = best_sentence[:first_eos[0]]
            final_output[sentence_id] = tgt_dict.string(best_sentence)

        # import pdb;pdb.set_trace()

    # Write the N-best list to its file
    n_best_list_path = args.n_best_list + '.txt'
    with open(n_best_list_path, 'w') as n_best_file:
        for sent_id, hypotheses in n_best_list.items():
            n_best_file.write(f"Sentence {sent_id}\n")
            for rank, (score, hypothesis) in enumerate(sorted(hypotheses, reverse=True)):
                n_best_file.write(f"Rank {rank + 1}: {hypothesis}\n")
            n_best_file.write('\n')

    # Write to file
    final_output_path = args.output
    with open(final_output_path, 'w') as out_file:
        for sent_id in sorted(final_output.keys()):
            out_file.write(final_output[sent_id] + '\n')
